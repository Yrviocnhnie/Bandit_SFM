#!/usr/bin/env python3
"""Generate synthetic v0 contextual-bandit data.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Generate first-step-only v0 bandit data without trigger-based scenarios")
...
def choose_base_state(rule: Dict[str, Any], scenario_id: str) -> List[str]:
    state_opts: List[str] = []
    for cond in rule.get("conditions", []):
        if cond.get("key") == "state_current":
            state_opts.extend(split_csv_values(cond.get("value")))
    if not state_opts:
    ...
    if not state_opts:
        if scenario_id in {"CAL_HAS_EVENTS", "MEETING_UPCOMING", "NO_MEETINGS"}:
            state_opts = ["office_working", "home_daytime_workday", "at_cafe_quiet"]
        elif scenario_id == "IN_MEETING":
            state_opts = ["office_working", "home_daytime_workday"]
        elif scenario_id == "REMOTE_MEETING":
            state_opts = ["home_daytime_workday", "at_cafe_quiet"]
        elif scenario_id == "DELIVERY_AT_OFFICE":
            state_opts = ["office_working", "office_arriving"]
        elif scenario_id in {"TRAIN_DEPARTURE", "FLIGHT_BOARDING", "HOTEL_CHECKIN"}:
            state_opts = ["at_rail_station", "at_airport", "at_transit_hub"]
        elif scenario_id == "MOVIE_TICKET":
            state_opts = ["at_social", "at_restaurant_other"]
        elif scenario_id == "HOSPITAL_APPOINTMENT":
            state_opts = ["at_health", "at_health_inpatient"]
        elif scenario_id == "RIDESHARE_PICKUP":
            state_opts = ["outdoor_walking", "stationary_unknown"]
        else:
            state_opts = ["stationary_unknown"]
    deduped: List[str] = []
    for s in state_opts:
        s = normalize_state(s)
        if s not in deduped:
...
def build_scenario_plan(spec: ScenarioSpec) -> ScenarioPlan:
    state_options = choose_base_state({"conditions": spec.conditions, "preconditions": spec.preconditions}, spec.scenario_id)
    precondition_options: List[str] = []
...
def main() -> None:
    ...
    metadata: Dict[str, Any] = {
        "generator": "generate_bandit_v0_firststep_no_triggers.py",
