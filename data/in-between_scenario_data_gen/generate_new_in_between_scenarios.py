'''
python generate_new_in_between_scenarios.py   --spec new_in_between_scenarios_spec.json   --output new_in_between_scenarios_1000_each.jsonl   --metadata metainfo/in_between_scenarios_1000_each_metadata.json   --samples-per-scenario 1000   --seed 7
'''
...
STATE_PROFILES = {
    ...
    'at_restaurant_lunch': {
        'ps_location': ['restaurant'], 'ps_time': ['lunch'], 'ps_dayType': ['workday','weekend','holiday'],
        'ps_motion': ['stationary','walking'], 'ps_phone': ['face_up','in_pocket','in_use','on_desk'],
        'ps_light': ['normal','bright'], 'ps_sound': ['normal','noisy'], 'networkType': ['wifi','cellular'],
        'isCharging': [0], 'activityState_by_motion': {'stationary':['sitting','standing'], 'walking':['standing','active']}
    },
    'outdoor_walking': {
...
def choose_calendar(spec: dict[str, Any], rng: random.Random) -> tuple[int,int,int,str]:
    ...
    if 'cal_eventCount' in fixed:
        event_count = int(fixed['cal_eventCount'])
    elif 'cal_event_count' in spec:
    ...
    if cal_has == 0 and event_count > 1:
        event_count = 1
    if cal_has == 0 and spec['scenario_id'] in {'HOME_DEEP_WORK_MORNING','GYM_TO_CAFE_RECOVERY','LATE_OFFICE_PRE_DEPARTURE_WRAPUP','HOME_WORK_DELIVERY_INTERRUPT','WEEKEND_CAFE_OUTING_PLANNING','OFFICE_TO_GYM_TRANSITION','LEAVE_OFFICE_WITH_ERRAND_STOP'}:
        event_count = 0
...
def generate_row(spec: dict[str, Any], idx: int, base_seed: int) -> dict[str, Any]:
    ...
    valid_states = [s for s in spec['allowed_states'] if any(x in STATE_PROFILES[s]['ps_time'] for x in spec['ps_time'])]
    if not valid_states:
        raise ValueError(f"{spec['scenario_id']} has no allowed_states compatible with ps_time")
    ...
    # scenario-level semantic biases to make the samples more obviously realistic
    sid = spec['scenario_id']
    if sid in {'QUIET_CAFE_BEFORE_MEETING', 'WEEKEND_CAFE_OUTING_PLANNING'}:
        motion = 'stationary' if 'stationary' in prof['ps_motion'] else motion
        activity = rng.choice(prof['activityState_by_motion'][motion])
        if phone == 'in_pocket':
            phone = rng.choice([x for x in prof['ps_phone'] if x in {'on_desk','face_up','in_use'}] or prof['ps_phone'])
    if sid == 'HOME_DEEP_WORK_MORNING':
        motion = 'stationary' if 'stationary' in prof['ps_motion'] else motion
        activity = rng.choice(prof['activityState_by_motion'][motion])
        net = 'wifi' if 'wifi' in prof['networkType'] else net
        if phone in {'in_pocket'}:
            phone = rng.choice([x for x in prof['ps_phone'] if x in {'on_desk','face_up','in_use'}] or prof['ps_phone'])
    if sid == 'LATE_OFFICE_PRE_DEPARTURE_WRAPUP' and 'stationary' in prof['ps_motion'] and rng.random() < 0.8:
        motion = 'stationary'
        activity = rng.choice(prof['activityState_by_motion'][motion])
        if phone == 'in_pocket':
            phone = rng.choice([x for x in prof['ps_phone'] if x in {'face_down','on_desk','in_use','face_up'}] or prof['ps_phone'])
    if sid == 'HOME_WORK_DELIVERY_INTERRUPT' and 'stationary' in prof['ps_motion'] and rng.random() < 0.7:
        motion = 'stationary'
        activity = rng.choice(prof['activityState_by_motion'][motion])
        net = 'wifi' if 'wifi' in prof['networkType'] else net
