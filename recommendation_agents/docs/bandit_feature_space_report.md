...
This is the final cleaned feature-space specification after the last dimension check.
...
- In **v0**, history features are **off**.
- In **v1**, history features are **on**.

## 2. Final dimension rules used in this report

To make the feature dimensions internally consistent, I applied these rules everywhere:

1. If a taxonomy already includes `unknown`, I do **not** add another extra unknown class.
2. Binary features stay strictly binary: `{0,1}` only.
3. If a feature can be missing and the base taxonomy does **not** include an unknown state, I add one fallback class.
4. You requested an explicit `unknown` class for these model features, so it is included where not already present:
   - `scenarioId`
   - `state_current`
   - `transportMode`
   - `activityState`
5. `cal_nextMinutes` is removed from the **final model input**, even though it still appears in a few handwritten rules.
6. Raw user name text is not used directly. It is represented by `user_id_hash_bucket`.
...
This table is about **rule usage frequency**, not the final keep/drop decision. So `cal_nextMinutes` is still shown here because it appears in the handwritten rules.
...
- For the model input, I add one extra fallback class: **`unknown`**
- So the **modeling dimension** for `state_current` becomes **65**
...
The final input features are grouped into:
...
| --- | --- | --- | --- | --- | --- | --- | ---: | --- |
| Scenario control | `scenarioId` | Yes | Yes | categorical | 65 scenario IDs + `unknown` | one-hot | 66 | Strong prior over valid recommendations; also useful as scenario-conditioned context. |
| Occurs in rules | `state_current` | Yes | Yes | categorical | 64 canonical `StateCode` values + `unknown` | one-hot | 65 | Main semantic summary of the current situation. |
...
| Auxiliary | `transportMode` | Yes | Yes | categorical | `walking, running, cycling, driving, transit, stationary, unknown` | one-hot | 7 | Stable mobility abstraction; `unknown` added because base taxonomy has none. |
| Auxiliary | `activityState` | Yes | Yes | categorical | `sitting, sleeping, standing, active, unknown` | one-hot | 5 | High-level activity summary; `unknown` added because base taxonomy has none. |
| Auxiliary | `activityDuration` | Yes | Yes | numeric | seconds >= 0 | 1 normalized scalar | 1 | Persistence of current activity state. |
| History (v1 only) | `hist_state_t-1` | No | Yes | categorical | 64 canonical `StateCode` + `unknown` + `none` | one-hot | 66 | Most recent previous state in the short trajectory. |
| History (v1 only) | `hist_state_t-2` | No | Yes | categorical | 64 canonical `StateCode` + `unknown` + `none` | one-hot | 66 | Second previous state in the short trajectory. |
| History (v1 only) | `hist_state_t-3` | No | Yes | categorical | 64 canonical `StateCode` + `unknown` + `none` | one-hot | 66 | Third previous state in the short trajectory. |
...
| History (v1 only) | `recent_accepted_ro_action_in_scenario` | No | Yes | categorical | 167 scenario-scoped R/O actions + `none` | one-hot | 168 | Core personalization signal for recent accepted RO behavior in the same scenario. |
...
| History (v1 only) | `recent_ro_feedback_score_same_scenario` | No | Yes | numeric | `[-1,1]` | 1 scalar | 1 | Aggregated recent reward for RO choices in the same scenario. |
...
| --- | --- | --- |
| `cal_nextMinutes` | Excluded | Low frequency in rules and probably not very useful compared with `cal_hasUpcoming`, `cal_eventCount`, and `cal_inMeeting`. |
| `ps_light` | Excluded | Dark/light effects are already captured well by the state encoder and dark substates. |
...
- **For v0**: the model sees the matched scenario, all rule-used features that matter, a small set of useful auxiliary signals, and user profile. That is enough to learn the current default policy from independent scenario snapshots.
...
- **Unknown is only added where it is actually needed**, which keeps the dimensions internally consistent.
...
| group | v0 dim | v1 dim |
| --- | ---: | ---: |
| Scenario control | 66 | 66 |
| Occurs in rules | 159 | 159 |
| Auxiliary | 44 | 44 |
| History (v1 only) | 0 | 401 |
| User profile | 45 | 45 |
| **Total** | **314** | **715** |
...
- **v0 input shape**: `(314,)`
- **v1 input shape**: `(715,)`
- Batch shapes are `(batch_size, 314)` and `(batch_size, 715)`

## 8. Output space

| model | phase | output space | output dim | notes |
| --- | --- | --- | ---: | --- |
| RO model | v0 supervised target | scenario-scoped default RO target (`scenarioId::default_actionId`) | 65 | One default R/O target per scenario in v4. |
| RO model | v1 online arm pool | scenario-scoped R/O candidate arm pool (`scenarioId::actionId`) | 167 | Rank only the arms valid for the current scenario. |
| App model | v0/v1 model head | app-category taxonomy | 10 | Predict app category, not a concrete app. |

### Practical interpretation

- **RO model**: use `scenarioId` + context to learn the default target in v0, then rank the full scenario-valid R/O arm pool in v1.
...
- **v0**: scenario-conditioned offline model with scenario control + rule-used features + auxiliary features + user profile, but **no history features**
...
- **`scenarioId` stays in the input vector**
- **`scenarioId` should also still be used as a serving-time candidate mask** so the agent only ranks actions valid for that scenario
...
## Appendix A. Scenario IDs (65 canonical + model fallback `unknown`)
