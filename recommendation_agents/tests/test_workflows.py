...
class WorkflowEvaluationTest(unittest.TestCase):
    ...
    def test_simulate_feedback_propagation_hard_assigned_local_cutoff_writes_threshold_and_margins(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            tmp_path = Path(tmp_dir)
            data_dir = tmp_path / "prepared"
            data_dir.mkdir()
            ...
            self.assertEqual(summary.similarity_thresholds, [0.0])
            self.assertEqual(len(summary.conditions), 1)
            condition = summary.conditions[0]
            self.assertEqual(condition["mode"], "hard-assigned-local-cutoff")
            self.assertEqual(condition["requested_n"], 2)
            self.assertEqual(condition["similarity_threshold"], 0.0)
            feedback_result = condition["feedback_results"][0]
            self.assertIn("same_scenario_outside_neighbors", feedback_result)
