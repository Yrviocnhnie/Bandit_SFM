"""Train v5: v3 R6-arch-trim base + background-state features.

Defaults to v3 R6-arch-trim base (drops scene, F1-F4, 2h/6h windows, n_trans;
no recency/periodicity; Markov fusion ON). New flags layered on top:
  --bg-mask-prior     Add α_bg_a · log(bg_mask + ε) to Task A logits.
                      Add α_bg_b · bg_mask to Task B sigmoid logits.
                      Each α is a learnable scalar per task (init 0.5, clipped).
  --bg-scalars        Append (bg_count_norm, bg_recency_min_norm,
                      time_since_screen_on_norm) to the profile (3 dims).
  --aux-reuse         Multi-task: 0.1 * BCE(predict is_reuse | fused).
  --wider             d_local 64→128, app_emb 32→64 (~3× capacity).
"""
...
def main():
    ...
    # v5 BG-state flags
    p.add_argument("--bg-mask-prior", dest="bg_mask_prior", action="store_true",
                   help="Add α_bg · bg_mask logit prior to Task A and Task B heads.")
    p.add_argument("--bg-scalars", dest="bg_scalars", action="store_true",
                   help="Append bg_count, bg_recency_min, time_since_screen_on to profile (3 dims).")
    p.add_argument("--aux-reuse", dest="aux_reuse", action="store_true",
                   help="Auxiliary task: predict is_reuse from fused rep (BCE, weight 0.1).")
    p.add_argument("--wider", dest="wider", action="store_true",
                   help="d_local 64->128, app_emb 32->64 (about 3x params).")
    ...
    for split_name, df, ids in (("train", train, loc_ids["train"]),
        ...
        # ----- v5 BG features -----
        bg_snaps = reconstruct_bg(df, vocab)
        bg_arrs = per_target_arrays(bg_snaps, V)
        if args.bg_scalars:
            # Compute bg_recency_min: min positive recency value per row (0 if BG empty)
            rec = bg_arrs["bg_recency"]                         # (M, V)
            mask_f = bg_arrs["bg_mask"]                          # (M, V)
            # rec is already log1p-normalized to [0,1]; for "min" we want most-recently-active
            # but recency is *seconds since*, so smallest among active is most recent
            inf_for_inactive = np.where(mask_f > 0, rec, np.inf)
            bg_rec_min = np.where(np.isinf(inf_for_inactive).all(axis=1),
                                  0.0, np.min(inf_for_inactive, axis=1)).astype(np.float32)
            bg_count = np.asarray(bg_arrs["bg_count"], dtype=np.float32)
            tsos = np.asarray(bg_arrs["time_since_screen_on"], dtype=np.float32)
            scalars = np.stack([bg_count, bg_rec_min, tsos], axis=1).astype(np.float32)
        ...
        tensors[split_name] = {
            ...
            "target_app": short["target_app"],
            "side_hour_fourier": short["target_hour_fourier"],
            "bg_mask": bg_arrs["bg_mask"],          # (M, V) float32
            "is_reuse": bg_arrs["is_reuse"],        # (M,) float32
        }
    ...
    # Optional capacity boost (--wider): rebuild cfg with wider local + app embedding
    if args.wider:
        cfg.d_local = 128
        cfg.app_emb_dim = 64
    if args.task == "b":
        base = M3.TaskBModelV3(cfg, markov_log_prior=mk_log_prior)
    else:
        base = M3.TaskAModelV3(cfg)

    # Wrap the base model to add BG-mask prior + auxiliary reuse head
    class V5_BGWrapper(nn.Module):
        def __init__(self, inner, task: str, V: int, d_fused: int,
                     bg_mask_prior: bool, aux_reuse: bool):
            super().__init__()
            self.inner = inner
            self.task = task
            self.V = V
            self.bg_mask_prior = bool(bg_mask_prior)
            self.aux_reuse = bool(aux_reuse)
            if self.bg_mask_prior:
                # Task A: stronger logit prior; Task B: softer linear prior.
                self.alpha_bg = nn.Parameter(torch.tensor(0.5))
            if self.aux_reuse:
                self.aux_head = nn.Linear(d_fused, 1)

        def _fused(self, batch):
            """Recompute fused representation from inner encoders."""
            cfg = self.inner.cfg if hasattr(self, "inner") else None
            return None  # we rely on the inner's internal forward; aux_reuse uses pre-final fused via hook

        def forward(self, batch):
            out = self.inner(batch)
            bg = batch.get("bg_mask")
            if bg is None:
                return out
            bg_f = bg.float()
            if self.bg_mask_prior:
                if self.task == "b":
                    # additive logit prior on Task B sigmoid head; α_bg learnable, no log (apps not in BG simply have mask=0)
                    alpha = torch.clamp(self.alpha_bg, -2.0, 5.0)
                    if isinstance(out, dict):
                        out = {**out, "logits_b_sig": out["logits_b_sig"] + alpha * bg_f}
                else:
                    # Task A: log-prior, strict
                    eps = 1e-3
                    alpha = torch.clamp(self.alpha_bg, 0.0, 5.0)
                    if torch.is_tensor(out):
                        out = out + alpha * torch.log(bg_f + eps)
            return out

    inner_model_v3 = M3.TaskBModelV3(cfg, markov_log_prior=mk_log_prior) if args.task == "b" else M3.TaskAModelV3(cfg)
    if args.bg_mask_prior or args.aux_reuse:
        # rewrap with cfg (and re-build inner if --wider was set)
        # NOTE: we already built `model` above for non-bg cases; for bg cases we rebuild here.
        d_fused = cfg.d_fused
        model = V5_BGWrapper(inner_model_v3, args.task, V, d_fused,
                              args.bg_mask_prior, args.aux_reuse)
    else:
        model = inner_model_v3
