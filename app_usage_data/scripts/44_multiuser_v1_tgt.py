"""Train v1 TGT-lite per user (2-layer Transformer, Fourier-hour gating)."""
...
def prep(df, vocab, scaler):
    ...
    return {
        ...
        "side": torch.as_tensor(h["target_hour_fourier"], dtype=torch.float32),
...
def evaluate(model, t, V):
    ...
    n = t["ta"].shape[0]
    if n == 0:
        return (metrics_a(np.zeros((0, V), dtype=np.float32), np.zeros(0, dtype=int)),
                metrics_b(np.zeros((0, V), dtype=np.float32), np.zeros((0, V), dtype=np.float32)))
    pa_chunks, sb_chunks = [], []
    with torch.no_grad():
        for i in range(0, n, BATCH):
            j = min(i + BATCH, n)
            out = model(t["ha"][i:j], t["hf"][i:j], t["hm"][i:j], t["side"][i:j])
...
def main():
    ...
    print(f"[v1 TGT-lite] training on {len(udirs)} users (epochs={EPOCHS})")
    all_results = []
    t0 = time.time()
    for set_name, uid, ud in udirs:
        ...
        try:
            ...
            r["set"] = set_name
            r["uid"] = uid
            with open(ud / "v1_tgt.json", "w") as f:
    ...
    out_agg = ROOT / "artifacts" / "multiuser" / "v1_tgt_aggregate.json"
