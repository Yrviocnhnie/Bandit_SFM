...
def metrics_b(sigs, win_counts):
    ...
    for i in range(n):
        ...
        topd_set = set(int(a) for a in topd)
        denom = float(gsize)
        rec5_list.append(len(top5_set & gt) / denom)
        rec_dyn_list.append(len(topd_set & gt) / denom)
        cov5_list.append(1.0 if gt.issubset(top5_set) else 0.0)
    if total_events == 0:
        base["n"] = int(n)
        return base
    return {
        "event_hit_at_1": float(hit1) / float(total_events),
        "event_hit_at_3": float(hit3) / float(total_events),
        "event_hit_at_5": float(hit5) / float(total_events),
        "event_hit_at_dyn": float(hit_dyn) / float(total_events),
