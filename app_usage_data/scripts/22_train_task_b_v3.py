"""Train TaskBModelV3 with v3 feature enrichment (+ optional Markov prior fusion)."""
...
from lib.v3.datasets_v3 import TaskBDatasetV3
...
def main():
    ...
    p.add_argument("--out", default="task_b_v3.pt")
    p.add_argument("--tag", default="task_b_v3")
    ...
    print(f"[task_b_v3] params={n_params}  profile_dim={profile_dim}  "
          f"flags: cat={args.use_category} loc={args.use_loc} "
          f"daypart={args.use_daypart} windows={args.use_windows} markov={args.use_markov}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(f"  ep{epoch:02d}  loss={train_loss / max(1, n_batches):.4f}  val EH@5={eval_metrics['event_hit_at_5']:.4f}  R@5={eval_metrics['recall_at_5']:.3f}  Cov@5={eval_metrics['coverage_at_5']:.3f}")
        if eval_metrics["event_hit_at_5"] > best_eh5 + 1e-6:
            best_eh5 = eval_metrics["event_hit_at_5"]
    ...
    torch.save({"state_dict": best_state, "cfg": vars(cfg), "best_eh5": best_eh5,
               ...
               out_ckpt)
    print(f"[train_task_b_v3] saved {out_ckpt}  best val EH@5={best_eh5:.4f}  elapsed={time.time() - t0:.1f}s")
    ...
    with open(out_results, "w") as f:
        json.dump({
            ...
            "test": test_metrics,
            "train_log": log,
            "alpha_markov_final": float(model.alpha_markov.item()) if (args.use_markov and hasattr(model, 'alpha_markov') and model.alpha_markov is not None) else None,
        }, f, indent=2)
    print(f"[train_task_b_v3] best val EH@5={best_eh5:.4f}  test EH@5={test_metrics['event_hit_at_5']:.4f}")
    return 0
