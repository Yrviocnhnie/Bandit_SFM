...
3. **`app_usage_data/`** — single-user next-app prediction demo with three tasks on 42 days of real HarmonyOS data:
   ...
   - **Task C — multi-user variant** (`REPORT_bgkill_multiuser.md` + `REPORT_bgkill_multiuser_c34.md`, scripts `50–55`): same problem on the **22-user cohort** at `/data00/ruiqing/app_forecasting/data/cleaned/{M_beta_Top30,top2000}/*.xlsx`. **One global model** trained on pooled bg rows (V_pool = 243, 625k train / 75k val / 65k test), with per-user feature stats (Markov / hour_freq / per-app inter-fg mean / lifetime stats) injected as feature inputs at scoring time via vectorized lookup keyed by `user_id_idx`. Listwise loss + metric groupbys use a global-anchor-id (`user_id_idx * 1e6 + per_user_anchor_id`) to keep different users' anchors distinct. Two eval slices: (A) the 22-user pooled bg_test, aggregated as mean / median / bootstrap CI across users; (B) cold-start single-user transfer onto the existing single-user bg_test. **c3.4 ablation (3 stages):** Stage 1 (c3p4_cheap, +F3/F5/F6/F7 numeric features, 41 dims) → +0.24 pp PR-AUC, gate-marginal. Stage 2 (c3p4_full, +F1 2-step Markov + F2 co-FG matrix, 44 dims) → −0.77 pp PR-AUC, regresses on sparse-cell noise. **Stage 3 (c3p3_cat, unchanged c3.3 numerics + 4-d learnable `cat_emb` over 11 categories, +44 params, +300 total) → +0.36 pp PR-AUC, only c3.4-family change clearing the +0.3 pp gate**; leads on PR-AUC (0.908), FK@.25 (0.116), MSR@.25 (0.470), WAKR@.25 (0.160), and P_kill at τ_keep (0.878). 63 pytest tests across 7 layers (54 base + 9 c3.4 fitter tests in `lib/bg_multi/tests/test_c34_features.py`) cover causality, per-user lookups, model IO, and metric ranges.
...
python scripts/52_train_task_c_multiuser.py --epochs 30 --patience 4                 # train all 7 picks on pooled data: original 4 (c3p3 / c3pro_reg / c3pro_listwise / c3pro_wide) + c3.4 (c3p4_cheap = 41 features F3/F5/F6/F7; c3p4_full = 44 features F1/F2; c3p3_cat = c3.3 + 4-d cat_emb arch change — gate-passer at +0.36 pp PR-AUC)
python scripts/52_train_task_c_multiuser.py --recipes c3p3_cat --epochs 30 --patience 4  # train just one recipe (e.g., the cat_emb gate-passer)
...
python -m pytest lib/bg_multi/tests/ -v                                              # 63 tests (54 base + 9 c3.4 fitter tests): causality, lookups, model IO, metric ranges
```
...
- `app_usage_data/REPORT_bgkill_multiuser_c34.md` — Task C multi-user c3.4 ablation (3 stages on top of c3.3): Stage 1/2 = F1–F7 numeric features (regress / marginal), Stage 3 = `cat_emb` architecture change (PASSES +0.3 pp gate at +0.36 pp PR-AUC, new headline on PR-AUC / FK@.25 / MSR@.25 / WAKR@.25 / P_kill)
...
- `lib/bg_multi/` (Task C — multi-user variant): `helpers.py` (`list_cohort_users`, `build_pooled_vocab` with min-count threshold + reserved `<PAD>/<UNK>/<RARE>`, `fit_per_user_stats` bundling Markov / hour_freq / per-app-inter-fg / lifetime stats shaped against the pooled vocab, `stack_per_user` for `(U, *)` vectorized lookup, `make_global_anchor_id` = `user_id_idx * 1e6 + per_user_anchor_id` for listwise loss + metric grouping). `c34_features.py` (c3.4 fitters: `fit_dow_hour_freq` per-user `(7, 24, V)` table for F3, `fit_app_popularity_bucket` global `{rare, medium, common}` for F5, `fit_cross_user_app_prior` leave-one-out for F6, `compute_last2_fg_for_anchors` causal lookup, `fit_two_step_markov_per_user` sparse `{(a_{t-2}, a_{t-1}) → (V,) probs}` for F1, `fit_co_fg_matrix_per_user` per-user `(V, V)` co-FG for F2). `tests/` (63 pytest tests across 7 layers; `test_c34_features.py` has 9 fitter unit tests). Build / train / eval scripts dispatch by `recipe["schema"]` via `_build_data_dispatch` in `scripts/52_train_task_c_multiuser.py`; cat_emb wired via `recipe["use_cat_emb"]` → `_TRAINER.SingleHeadMLP(use_cat_emb=True)` + `train_one_cat` fork + `_PairDSCat` dataset forwarding `cat_idx` through the forward.
