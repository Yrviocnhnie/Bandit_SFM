...
- **Track B — threshold-based** (`41_threshold_metrics.py`): per-(anchor, app) yes/no kill at threshold τ → FKR / SKR / F1 / Acc / **MCC**. τ\* picked by argmax F1 on val (49-quantile sweep) and frozen on test; also a fixed-τ sweep at τ ∈ {0.30, 0.40, 0.50, 0.60, 0.70} for trained models + Random. Closed-form baselines are excluded from Track B (their score scales aren't probabilities; F1-optimal τ lands at degenerate corners). MCC is the headline because the kill class is 78 % — F1 / Accuracy compress under that imbalance.
...
- `lib/bg/` (Task C): `background_state.py` (B(t) state machine — replays the full event stream and snapshots the BG set at each anchor with 2 h staleness cutoff), `features_bg.py` (per-anchor feature extractors, label generators), `baselines_bg.py` (closed-form baselines), `models_bg.py` (per-(anchor, app) MLP architectures, dual-head + single-head variants), `metrics_bg.py` (Track A: FalseKillRate@r, MemorySaveRate@r, PR-AUC, ROC-AUC, NDCG, Pareto · Track B: `compute_threshold_metrics` for FKR/SKR/F1/Acc/MCC at a given τ, `find_best_tau_by_f1` for the 49-quantile sweep, `compute_positive_only_metrics` for WAKR/PosRank/PosScoreNorm). Train-only stats (`hour_freq`, `app/cat_lifetime_share`, `cat_markov`, `per_app_inter_fg_mean`, etc.) are fit at training time inside `scripts/34_train_task_c_h60.py` and `scripts/36_train_c3_grid.py`.
